...
workflow EvidenceMerging {
  ...
  Int PE_disk_gb = overhead_gb + ceil((length(samples) * select_first([PE_size_mb, 200])) / 1000)
  Int SR_disk_gb = overhead_gb + ceil((length(samples) * select_first([SR_size_mb, 600])) / 1000)
  Int BAF_disk_gb = overhead_gb + ceil((length(samples) * select_first([BAF_size_mb, 50])) / 1000)
