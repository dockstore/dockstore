...
public class GitLabSourceCodeRepo extends SourceCodeRepoInterface {
        ...
        try {
            ...
            tagList.forEach(tag -> {
                if (versionName.isEmpty() || Objects.equals(versionName.get(), tag.getName())) {
                    ...
                    handleVersionOfWorkflow(repositoryId, workflow, existingWorkflow, existingDefaults, repositoryId, tag.getName(), Version.ReferenceType.TAG, committedDate, commitId, specs, hardRefresh);
                }
            });
            branches.forEach(branch -> {
                if (versionName.isEmpty() || Objects.equals(versionName.get(), branch.getName())) {
                    ...
                    handleVersionOfWorkflow(repositoryId, workflow, existingWorkflow, existingDefaults, repositoryId, branch.getName(),
                            Version.ReferenceType.BRANCH, committedDate, commitId, specs, hardRefresh);
                }
            });
        } catch (IOException e) {
    ...
    private void handleVersionOfWorkflow(String repositoryId, Workflow workflow, Optional<Workflow> existingWorkflow,
        Map<String, WorkflowVersion> existingDefaults, String id, String branchName, Version.ReferenceType type, Date committedDate, String commitId, WorkflowSpecifications specs, boolean hardRefresh) {
        if (toRefreshVersion(commitId, existingDefaults.get(branchName), hardRefresh)) {
            ...
            version = combineVersionAndSourcefile(repositoryId, sourceFile, workflow, identifiedType, version, existingDefaults, specs);
