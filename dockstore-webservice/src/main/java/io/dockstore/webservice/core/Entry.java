...
@NamedQueries({
    @NamedQuery(name = "Entry.getGenericEntryById", query = "SELECT e from Entry e WHERE :id = e.id"),
        ...
        @NamedQuery(name = "io.dockstore.webservice.core.Entry.getVersions", query = "SELECT new io.dockstore.webservice.core.dto.TrsToolVersion(wv.id, w.id, vm.author, vm.verified, wv.name, wv.frozen, wv.lastModified, vm.hidden) from Workflow w join w.workflowVersions wv left join wv.versionMetadata vm where w.id in :ids"),
        ...
        @NamedQuery(name = "io.dockstore.webservice.core.Entry.getImagesForVersions", query = "SELECT new io.dockstore.webservice.core.dto.TrsImageDTO(i.id, wv.id, i.imageRegistry, i.imageID, i.repository, i.checksums) from WorkflowVersion wv join wv.images i where wv.id in :ids"),
