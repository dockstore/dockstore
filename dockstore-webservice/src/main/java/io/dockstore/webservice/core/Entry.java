...
@NamedQueries({
    @NamedQuery(name = "Entry.getGenericEntryById", query = "SELECT e from Entry e WHERE :id = e.id"),
        ...
        @NamedQuery(name = "io.dockstore.webservice.core.Entry.findCategoryNamesByEntryId", query = "select col.name from Collection col join col.entries as entry join col.organization as organization where entry.entry.id = :entryId and organization.status = 'SPECIAL'"),
