...
@NamedQueries({
    @NamedQuery(name = "Entry.getGenericEntryById", query = "SELECT e from Entry e WHERE :id = e.id"),
        ...
        @NamedQuery(name = "io.dockstore.webservice.core.Entry.getImagesForVersions", query = "SELECT new io.dockstore.webservice.core.dto.TrsImageDTO(i.id, wv.id, i.imageRegistry, i.imageID, i.repository, i.tag, i.checksums) from WorkflowVersion wv join wv.images i where wv.id in :ids"),
