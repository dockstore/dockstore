...
# get build secret
RUN --mount=type=secret,id=COLLAB_DEPLOY_TOKEN,env=COLLAB_DEPLOY_TOKEN
...
RUN wget --user=dockstore-bot --password="$COLLAB_DEPLOY_TOKEN" -P /root/.dockstore/language-plugins https://artifacts.oicr.on.ca/artifactory/collab-release/com/github/galaxyproject/dockstore-galaxy-interface/dockstore-galaxy-interface/${galaxy_plugin_version}/dockstore-galaxy-interface-${galaxy_plugin_version}.jar

# Include snakemake language plugin
ARG snakemake_plugin_version=0.0.5
RUN wget --user=dockstore-bot --password="$COLLAB_DEPLOY_TOKEN" -P /root/.dockstore/language-plugins https://artifacts.oicr.on.ca/artifactory/collab-release/io/dockstore/snakemake-language-interface/${snakemake_plugin_version}/snakemake-language-interface-${snakemake_plugin_version}.jar
