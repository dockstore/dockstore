...
public class UserResourceIT extends BaseIT {
    ...
    public void testSelfDestruct() throws ApiException {
        ...
        // refresh targeted
        workflowsApi.refreshWorkflow(workflowByPath.getId());
        ...
        // publish one
        workflowsApi.publishWorkflow(workflowByPath.getId(), SwaggerUtility.createPublishRequest(true));
        ...
        // then unpublish them
        workflowsApi.publishWorkflow(workflowByPath.getId(), SwaggerUtility.createPublishRequest(false));
        assertTrue(userApi.getExtendedUserData().isCanChangeUsername());
    ...
    public void testWizardEndpoints() throws ApiException {
        ...
        workflowsApi.refreshWorkflow(ghWorkflow.getId());
        repositories = userApi.getUserOrganizationRepositories(SourceControl.GITHUB.name(), "dockstoretesting");
        assertTrue(repositories.size() > 0);
        assertTrue(
    ...
    public void testLoggedInHomepageEndpoints() {
        ...
        Workflow refreshedWorkflow = workflowsApi.refreshWorkflow(workflow.getId());
