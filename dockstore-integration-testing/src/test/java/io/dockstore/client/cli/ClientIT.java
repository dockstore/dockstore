...
public class ClientIT extends BaseIT{
    ...
    /**
     * Tests notifications webhook
     * @throws IOException
     */
    @Test
    public void launchCWLToolWithNotifications() throws IOException {
        final String firstTool = ResourceHelpers.resourceFilePath("dockstore-tool-helloworld.cwl");

        // Tests if an error is displayed when UUID is specified with no webhook URL
        Client.main(
                new String[] { "--config", TestUtility.getConfigFileLocation(true), "tool", "launch", "--local-entry", firstTool,
                        "--json", "https://raw.githubusercontent.com/ga4gh/dockstore/f343bcd6e4465a8ef790208f87740bd4d5a9a4da/dockstore-client/src/test/resources/test.cwl.json", "--uuid", "potato", "--debug" });
        ...
        Client.main(
                new String[] { "--config", TestUtility.getConfigFileLocationWithInvalidNotifications(true), "tool", "launch", "--local-entry", firstTool,
                        "--json", "https://raw.githubusercontent.com/ga4gh/dockstore/f343bcd6e4465a8ef790208f87740bd4d5a9a4da/dockstore-client/src/test/resources/test.cwl.json", "--uuid", "potato", "--debug" });
        log = systemOutRule.getLog();
        ...
        Client.main(
                new String[] { "--config", TestUtility.getConfigFileLocationWithValidNotifications(true), "tool", "launch", "--local-entry", firstTool,
                        "--json", "https://raw.githubusercontent.com/ga4gh/dockstore/f343bcd6e4465a8ef790208f87740bd4d5a9a4da/dockstore-client/src/test/resources/test.cwl.json", "--uuid", "potato", "--debug" });
        log = systemOutRule.getLog();
        Assert.assertTrue(log.contains("Sending notifications message"));
        Assert.assertTrue(!log.contains("Can not resolve webhook URL"));
        ...
        Client.main(
                new String[] { "--config", TestUtility.getConfigFileLocationWithValidNotifications(true), "tool", "launch", "--local-entry", firstTool,
                        "--json", "https://raw.githubusercontent.com/ga4gh/dockstore/f343bcd6e4465a8ef790208f87740bd4d5a9a4da/dockstore-client/src/test/resources/test.cwl.json", "--debug" });
        log = systemOutRule.getLog();
        Assert.assertTrue(!log.contains("Sending notifications message"));
        Assert.assertTrue(!log.contains("Can not resolve webhook URL"));
        clearLogs();
