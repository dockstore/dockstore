...
public enum DescriptorLanguage {
    ...
    CWL("CWL", "Common Workflow Language", FileType.DOCKSTORE_CWL, FileType.CWL_TEST_JSON, false, false,
        ImmutableSet.of("cwl", "yaml", "yml"), true, true) {
        @Override
    ...
    WDL("WDL", "Workflow Description Language", FileType.DOCKSTORE_WDL, FileType.WDL_TEST_JSON, false, false, ImmutableSet.of("wdl"), true, true) {
        @Override
        public boolean isRelevantFileType(FileType type) {
            return super.isRelevantFileType(type) || type == FileType.DOCKERFILE || type == FileType.DOCKSTORE_YML;
        }
    ...
    GXFORMAT2("gxformat2", "Galaxy Workflow Format 2", FileType.DOCKSTORE_GXFORMAT2, FileType.GXFORMAT2_TEST_FILE, false, true,
        ImmutableSet.of("ga", "yaml", "yml"), true, false) {
        @Override
        public boolean isRelevantFileType(FileType type) {
    ...
    SWL("SWL", "Silly Workflow Language", FileType.DOCKSTORE_SWL, FileType.SWL_TEST_JSON, false, true, ImmutableSet.of("swl"), false, false) {
        @Override
        public boolean isRelevantFileType(FileType type) {
            return super.isRelevantFileType(type);
        }
    },
    NEXTFLOW("NFL", "Nextflow", FileType.NEXTFLOW_CONFIG, FileType.NEXTFLOW_TEST_PARAMS, false, false, ImmutableSet.of("config"), true, false) {
        @Override
        public boolean isRelevantFileType(FileType type) {
    ...
    SERVICE("service", "generic placeholder for services", FileType.DOCKSTORE_SERVICE_YML, FileType.DOCKSTORE_SERVICE_TEST_JSON, true,
        false, ImmutableSet.of("yml"), true, false) {
        @Override
        public boolean isRelevantFileType(FileType type) {
    ...
    DescriptorLanguage(final String shortName, final String friendlyName, final FileType fileType, final FileType testParamType, final boolean serviceLanguage, final boolean pluginLanguage, final ImmutableSet<String> defaultPrimaryDescriptorExtensions, final boolean supportsHosted, final boolean supportsChecker) {
